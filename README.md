<p align="center">
  <img src="docs/media/demo.gif" alt="Day One Chef gameplay" width="640">
</p>

<h1 align="center">Day One Chef</h1>

<p align="center">
  <strong>한 줄 한국어 지시로 신입 인턴에게 요리를 가르치는 미니게임.</strong>
  <br>
  사수가 빠뜨린 단계는 인턴도 빠뜨린다. "넵!" 외치고 그대로 시연하면 손님이 한 줄 평을 남긴다.
  <br><br>
  <a href="#게임-소개">소개</a> &middot;
  <a href="#아키텍처">아키텍처</a> &middot;
  <a href="#기술-스택">기술 스택</a> &middot;
  <a href="#핵심-코드">핵심 코드</a> &middot;
  <a href="#설계-문서">설계 문서</a> &middot;
  <a href="#빌드와-실행">빌드와 실행</a>
</p>
...
손님이 `햄버거 주세요!` 외치면 플레이어는 한국어 한 줄(≤80자)로 인턴에게 만드는 법을 가르친다. 인턴은 그 지시를 verb 단위 행동으로 쪼개 주방에서 그대로 시연하고, 결과를 본 손님이 한 줄 평을 남긴다.

**Core loop:** 주문 등장 → 한국어 지시 → Gemini call #1이 행동 JSON 생성 → Unity가 시연 + 이벤트 로그 기록 → Gemini call #2가 손님 시점에서 판정 → Flutter 우측 패널에 결과 누적 → 5라운드 후 세션 결산.

**5라운드 코스:** 토스트 → 계란찜 → 샐러드 → 감자튀김 → 햄버거. 사람이라면 자연스럽게 빠뜨리지 않을 단계들("씻기", "껍질 깨기", "썰기")을 인턴은 사수가 말 안 하면 그냥 생략한다. 그 갭이 코미디 포인트다.
...
**핵심 설계 원칙:** Flutter ↔ Unity WebView 양방향 브릿지가 이 프로젝트의 1차 설계 면이다. 라운드 결과, 세션 종료, 리셋 명령은 모두 단일 `FlutterBridge` JS 채널 + `BridgeReceiver` Unity GameObject 한 쌍을 통해서만 흐른다 (ADR-0002).
...
**Unity Hub에서 열기:** "Add project from disk"에서 `day-one-chef/game/`을 선택한다 (리포 루트 아님).
...
**핵심 루프 완료, 폴리시 단계.** 5라운드 코스가 끝까지 돌고 Gemini 2회 호출, 브릿지, 결산 모달까지 연결됐다.
...
남은 폴리시: 에셋, 사운드, 톤 마무리, 모바일 IME 검증.
...
셸은 부팅 시 로컬 `shelf` 서버를 띄워 Unity WebGL 빌드를 정적 서빙하고, `/gemini` 경로로 Google AI API를 프록시한다. API 키는 `--dart-define=GEMINI_API_KEY=...` 로 주입한다.

**Vercel 웹 빌드**

Unity WebGL 산출물(Brotli 압축)만 배포한다. 셸 없이 동작하고 IME 폴백 + 브릿지 stub을 포함한다.
